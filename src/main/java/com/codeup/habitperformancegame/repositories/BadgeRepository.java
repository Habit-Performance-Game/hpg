...
public interface BadgeRepository extends CrudRepository<Badge,Long> {
    ...
    @Query(value = "select * from badges where id NOT IN (select badge_id from user_badges where user_id =?1) AND type = 'clan'", nativeQuery = true)
    List<Badge> clanNotAdded(long id);

    @Query(value = "select * from badges where id IN (select badge_id from user_badges where user_id =?1) AND type = 'clan'", nativeQuery = true)
